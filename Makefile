# Convenience wrapper around CMake/CTest workflows for parcel.
...
help:
	...
	@echo "  make example           Run the parcel_hello example"
	@echo "  make examples          Build and run every parcel_* example (fails on any non-zero exit)"
	...
	@echo ""
	@echo "  make ci                Run the full pre-push gate: format-check + tidy + test + sanitize + release"
	@echo ""
...
.PHONY: example
example: build
	$(BUILD_DIR)/examples/parcel_hello

# Build and run every parcel_* example. Mirrors the "Run examples" CI step:
...
examples: build
	@set -e; \
	failures=""; \
	for ex in $(BUILD_DIR)/examples/parcel_*; do \
		if [ -x "$$ex" ] && [ ! -d "$$ex" ]; then \
			name=$$(basename "$$ex"); \
			echo "=== $$name ==="; \
			if ! "$$ex" >/dev/null; then \
...
.PHONY: sanitize
sanitize:
	$(CMAKE) -S . -B build-san $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DPARCEL_ENABLE_SANITIZERS=ON
	$(CMAKE) --build build-san -j $(JOBS)
	$(CTEST) --test-dir build-san --output-on-failure

.PHONY: tidy
tidy:
	$(CMAKE) -S . -B build-tidy $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DPARCEL_ENABLE_CLANG_TIDY=ON
	$(CMAKE) --build build-tidy -j $(JOBS)

.PHONY: tidy-fix
tidy-fix:
	$(CMAKE) -S . -B build-tidy $(CMAKE_GEN_FLAG) -DCMAKE_BUILD_TYPE=Debug -DPARCEL_ENABLE_CLANG_TIDY=ON -DPARCEL_CLANG_TIDY_FIX=ON
	$(CMAKE) --build build-tidy -j $(JOBS)
...
coverage:
	$(CMAKE) -S . -B build-coverage $(CMAKE_GEN_FLAG) \
		-DCMAKE_BUILD_TYPE=Debug -DPARCEL_ENABLE_COVERAGE=ON
	$(CMAKE) --build build-coverage -j $(JOBS)
	rm -f build-coverage/*.profraw build-coverage/parcel.profdata
	LLVM_PROFILE_FILE="$(CURDIR)/build-coverage/parcel-%p.profraw" \
		$(CTEST) --test-dir build-coverage --output-on-failure
	$(LLVM_PROFDATA) merge -sparse build-coverage/*.profraw \
		-o build-coverage/parcel.profdata
	$(LLVM_COV) report build-coverage/tests/parcel_tests \
		-instr-profile=build-coverage/parcel.profdata \
		-ignore-filename-regex='(_deps|tests)/'
	$(LLVM_COV) show build-coverage/tests/parcel_tests \
		-instr-profile=build-coverage/parcel.profdata \
		-ignore-filename-regex='(_deps|tests)/' \
...
.PHONY: docs
docs:
	$(CMAKE) -S . -B build-docs $(CMAKE_GEN_FLAG) -DPARCEL_BUILD_DOCS=ON
	$(CMAKE) --build build-docs --target parcel_docs -j $(JOBS)
