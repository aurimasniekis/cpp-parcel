...
| `parcel_intrusive_struct_demo`   | `intrusive_struct_demo.cpp`   | `SelfStructCell<Derived>` + a CRTP `BaseEvent` that carves out its own `"s:event:…"` wire namespace.                | `SelfStructCell`, `id_join_lit_v`, `prefixed_id_v`, `FixedString`                                                    |
